...
async function selectChat(chatId) {
  try {
    await invoke("set_current_chat", { chatId });
    currentChatId = chatId;
    const messages = await invoke("read_chat_log", { chatId });
...
// Live agent thought process: the span we append step-by-step updates to (set during sendMessage).
let agentLiveSpan = null;

function appendAgentStep(step, thought, action, result) {
  if (!agentLiveSpan) return;
  const block = document.createElement("div");
  block.className = "agent-step";
  block.innerHTML = `<strong>Step ${step}</strong> — ${escapeHtml(thought || "")}<br><em>Action: ${escapeHtml(action || "")}</em>${result ? `<br>→ ${escapeHtml(result)}` : ""}`;
  agentLiveSpan.appendChild(block);
  chatMessages.scrollTop = chatMessages.scrollHeight;
}
...
async function sendMessage() {
  ...
  try {
    ...
    // Placeholder bot message so we can stream agent thought process into it
    const { span } = appendMessage("Working on it…", false);
    agentLiveSpan = span;
    const liveContainer = document.createElement("div");
    liveContainer.className = "agent-thought-process";
    span.appendChild(liveContainer);
    agentLiveSpan = liveContainer;
    ...
    // Replace live area with final reply (includes full thought process)
    agentLiveSpan = null;
...
// Listen for desktop agent step events (thought process streamed from backend)
window.__TAURI__.event.listen("desktop-agent-step", (e) => {
  const p = e.payload;
  if (p && agentLiveSpan) {
    appendAgentStep(p.step, p.thought, p.description || p.action, p.done ? "Done." : p.result);
  }
});
